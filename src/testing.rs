...
pub(crate) fn span_of(source: &str, text: &str, nth: usize) -> Span {
    let start = source
        .match_indices(text)
        .nth(nth)
...
pub(crate) fn spans(source: &str) -> impl FnMut(&str) -> Span + '_ {
    let mut offset = 0;
    move |text| {
...
pub(crate) fn name(text: &str, span: Span) -> ast::Name {
    ast::Name {
        text: text.to_string(),
        span,
    }
...
pub(crate) fn lex_parse(source: &str) -> ast::Program {
    ...
    let program = parse(source, &tokens, &mut diags).expect("parsing succeeded");
...
pub(crate) fn lex_parse_check(source: &str) -> typed_ast::Program {
    let mut diags = Diagnostics::default();
    let program = check(&lex_parse(source), &mut diags).expect("checking succeeded");
    assert!(diags.is_empty());
    program
}
